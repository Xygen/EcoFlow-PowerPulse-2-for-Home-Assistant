...
class EcoFlowMQTTClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._subscription_results: dict[str, int] = {}
    ...
    def subscription_results(self) -> dict[str, int]:
        ...
        return dict(self._subscription_results)

    def _subscribe(self, client: mqtt.Client, topic: str, qos: int, label: str) -> None:
        """Subscribe and retain only a safe label plus the broker result code."""
        try:
            result = client.subscribe(topic, qos=qos)
            rc = result[0] if isinstance(result, tuple) else getattr(result, "rc", 0)
            self._subscription_results[label] = int(rc)
            if int(rc) != 0:
                self._log_issue(
                    "warning",
                    "MQTT subscription rejected for %s (rc=%s)",
                    label,
                    rc,
                )
        except Exception as exc:
    ...
    def _on_connect(self, client, userdata, flags, rc, properties=None):
        ...
        if rc_val == 0:
            ...
            # Passive command discovery. These subscriptions never transmit;
            # publish() remains the hard listen-only choke point. PowerPulse
            # commands may be routed through app-auth or device-facing topics,
            # especially when the charger is linked to PowerOcean.
            open_set = f"/open/{self._cert_account}/{self._device_sn}/set"
            open_set_reply = f"/open/{self._cert_account}/{self._device_sn}/set_reply"
            device_set = f"/app/device/property/{self._device_sn}/set"
            ...
            if self._user_id:
                ...
                # Passive reverse-engineering aid: observe official-app SET
    ...
    def _on_message(self, client, userdata, msg):
        ...
        if (
            ...
            return
        _LOGGER.debug("MQTT msg: %s (%d bytes)", self._masked_topic(msg.topic), len(msg.payload))
        try:
    ...
    def publish(self, topic: str, payload: str | bytes, qos: int = 1) -> bool:
        ...
        try:
            ...
            _LOGGER.error("Publish failed (%s): %s", self._masked_topic(topic), exc)
            return False
