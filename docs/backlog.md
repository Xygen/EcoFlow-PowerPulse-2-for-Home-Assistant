...
| `CTRL-01` | Immediate live validation | Live-validate the dev29 disabled-by-default Start and Stop buttons. The implementation uses the confirmed `241/100` selectors, preserves the opaque descriptor, requires a fresh heartbeat before publishing, rejects Start while unplugged, and confirms only from a newer heartbeat after the matching reply. | Reversible HA tests distinguish unplugged rejection, successful Start, successful Stop, and paused/completed outcomes where applicable; no stale heartbeat or unrelated SET reply can produce a false success. |
