...
| `PHASE-01` | Research; beta instrumentation ready | Establish the provider `phaseSpecified` mapping so phase writes can eventually be confirmed while the direct path sleeps. A 2026-08-28 HA sequence reconfirmed the direct mapping (`0` Auto, `1` one phase, `2` three phase) with roughly one-second device readback, but produced six direct and zero provider confirmations. Version `0.1.1-beta.1` retains the direct, parent-accessory provider, and device-detail provider observations separately with timestamps. Until the provider values are isolated, phase control requires fresh direct `phase_mode` readback. | Install the beta, then isolate Auto, one-phase, and three-phase provider values through controlled app changes and verify them against independent app/device state before provider phase confirmation is implemented. |
| `SAFE-01` | Beta validation | Custom- and Smart-mode charging tests completed the interlock matrix. The official app hides the Custom-current slider and makes Smart ready-by, target type, and target value uneditable while charging. Version `0.1.1-beta.1` blocks Custom current and all Smart setting keys in both entity availability and the backend, in addition to the five dev28 locks. Plug-and-Play, battery, screen, LED, and both brightness controls remain available as previously confirmed. | Install the beta and verify during active Custom and Smart sessions that every locked HA entity is unavailable, direct service calls fail closed, and the app-confirmed usable controls remain available. |
