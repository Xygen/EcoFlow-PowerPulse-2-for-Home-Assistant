...
Current implementation baseline: `0.1.0-dev28`.
...
| `CTRL-01` | Next control build | Implement Start and Stop using the live-confirmed PowerOcean-routed `241/100` command: field 2 value `1` stops and value `2` starts. Preserve the captured field-1 accessory descriptor structurally without logging its opaque value, reject Start while unplugged, and use independent heartbeat state rather than the SET reply alone as the outcome. | Reversible HA tests distinguish unplugged rejection, successful Start, successful Stop, and any observed paused/completed outcomes; each command has a same-sequence reply plus fresh heartbeat confirmation and no stale optimistic state. |
