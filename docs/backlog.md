...
| `STREAM-01` | Research | Determine which app-side session action reactivates direct C376 `241/44`. A stale-stream test proved that renewing the three existing subscriptions does not wake it, while opening only the PowerPulse overview in the EcoFlow app does; no C376 GET or SET was visible on the subscribed topics. Next isolate an app connection/subscription, an unobserved MQTT topic, and an HTTP/backend request before implementing automatic recovery. | A controlled capture identifies a reproducible request or session action that HA can safely emulate; merely reopening the app is not sufficient protocol evidence. |
