...
| `CTRL-01` | Next live capture after dev28 | Repeat official-app Stop and Start with diagnostic schema 11, then decode the differing selector inside the shared PowerOcean-routed `241/100` body before implementing controls. The first connected-vehicle pair confirmed the command tuple, target, acknowledgement, state transitions, and new-session reset, but schema 10 omitted the 25-byte body. | Privacy-safe diagnostics distinguish Start from Stop without retaining the accessory descriptor; independent readback and reversible HA tests distinguish unplugged, plugged-in, charging, paused, completed, and rejected outcomes where observed. |
