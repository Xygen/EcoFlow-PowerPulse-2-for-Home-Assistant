...
| `DATA-09` | Research / potentially low effort | Determine why `sensor.ecoflow_powerocean_wallbox_ladeleistung` from the other EcoFlow integration tracks the actual charging power substantially more accurately than this integration's `sensor.powerpulse_2_ladeleistung`. Identify its precise source field or calculation, update cadence, scaling, phase handling, freshness behavior, and any filtering or fallback logic. Then assess whether the same approach can be adopted in this standalone integration with a small, maintainable change and without adding a runtime dependency on the other integration. | Simultaneous live samples compare both sensors with the EcoFlow app across stable and changing loads; diagnostics or source-code evidence explains the difference. A bounded implementation proposal names the required local data path and confirms accuracy, update rate, standalone operation, and backward-compatible entity semantics before changing the production sensor. |
