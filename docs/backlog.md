...
| `SAFE-01` | Implementation complete; live validation pending | Custom- and Smart-mode charging tests completed the interlock matrix. The official app hides the Custom-current slider and makes Smart ready-by, target type, and target value uneditable while charging. The unreleased implementation now blocks Custom current and all Smart setting keys in both entity availability and the backend, in addition to the five dev28 locks. Plug-and-Play, battery, screen, LED, and both brightness controls remain available as previously confirmed. | Install the next build and verify during active Custom and Smart sessions that every locked HA entity is unavailable, direct service calls fail closed, and the app-confirmed usable controls remain available. |
