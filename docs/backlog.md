...
| `STREAM-01` | Next idle window | Use dev25's disabled diagnostic reconnect after both stream sensors are stale. It rebuilds only the C376 WSS client with a new Client ID, subscribes normally, and publishes nothing. Simple re-subscription failed, while opening only the EcoFlow home/device list restarted `241/44` immediately and heartbeat `2/33` about 47–54 seconds later; no C376 GET or SET was visible. If reconnect also fails, isolate an app-specific subscription, unobserved topic, or HTTP/backend request. | A stale baseline and safe reconnect trace establish whether a fresh C376 client session alone restarts both streams without a device command. Only a confirmed result may become automatic recovery. |
