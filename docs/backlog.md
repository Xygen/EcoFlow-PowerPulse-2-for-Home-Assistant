...
| `STREAM-01` | Next idle window | Determine what reactivates the direct C376 `241/44` stream. First use the dev24 diagnostic re-subscribe button without restarting HA; if it does not resume, open the EcoFlow app without changing a setting and compare the retained GET/SET traffic. | A genuinely stale baseline plus timestamps establishes whether C376 subscription renewal, a visible app publish, or an app connection/subscription with no observable publish precedes resumed `241/44` reports. Only then consider automatic recovery or a GET command. |
