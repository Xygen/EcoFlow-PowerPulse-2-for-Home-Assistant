...
| `DATA-07` | Research | Evaluate a diagnostic charging-power sensor calculated from phase voltage and current. In the observed one-phase session, `230.2 V * 5.71 A = 1314 W` was closer to the app/provider value of about `1338 W` than the current direct power value of `1272.5 W`. This would not improve freshness: both inputs currently arrive in the same direct heartbeat at roughly 60-second intervals. Do not use the present maximum-phase summaries as a general three-phase power calculation. | One-phase and three-phase charging captures determine the required per-phase calculation and power-factor treatment; accuracy and update cadence are compared with the app/provider and direct power field before deciding whether to add or replace an entity. |
